...
def transform_date():
    transform_data_s3_bucket = "s3://redfin-data-bucket/transformed-data/redfin_data.parquet"
    #Read the redfin data from the s3 bucket
    redfin_data = spark.read.csv("s3://redfin-data-bucket/raw-data/city_market_tracker.tsv000.gz", header=True, inferSchema=True, sep= "\t")
